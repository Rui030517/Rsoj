<!-- <template>
    <div id="workQuestionView">

        <a-row :gutter="[24,24]">
            <a-col :md="12" :xs="24">

                <a-tabs>
                    <a-tab-pane key="quesion">
                        <template #title>
                            <icon-calendar/> 题目浏览
                        </template>
                        <a-card v-if="question" :title="question.title" hoverable>
                            ...
                            <template #extra>
                                <a-space>
                                    <a-space wrap>
                                        <a-tag v-for="(tag, index) of question.tags" :key="index" color="#7bc616" >{{ tag }}</a-tag>
                                    </a-space>
                                </a-space>
                            </template>

                            <a-divider />
                            <a-descriptions title="题目样例" :column="{xs:1, md:2, lg:3}">
                                <a-descriptions-item label="输入样例">
                                    <a-tag>{{ question.judgeConfig?.timelimit }}</a-tag>
                                </a-descriptions-item>
                                <a-descriptions-item label="输出样例">
                                    <a-tag>{{ question.judgeConfig?.memorylimit }}</a-tag>
                                </a-descriptions-item>
                            </a-descriptions>
                        </a-card>
                    </a-tab-pane>
                    <a-tab-pane key="comment" :disabled="true">
                        <template #title>
                            <icon-clock-circle/> 评论区
                        </template>
                            评论区
                    </a-tab-pane>
            ...
            <a-col :md="12" :xs="24">
                <a-form :model="form" layout="inline">
                    <a-form-item field="title" label="编程语言" style="min-width: 240px;">
                        <a-select :style="{width:'320px'}" v-model="form.language" placeholder="选择您的编程语言">
                ...
                <a-divider size="0"/>
                <a-button type="primary" style="min-width: 100px;" @click="doSubmit">提交代码</a-button>
            </a-col>
        </a-row>
    </div>
</template>
...
import { QuestionControllerService, QuestionSubmitAddRequest } from "../../../generated";
...
.arco-space-horizontal .arco-space-item{
 ...
 <template>
  ...
  import { useStore,withDefaults,defineProps } from "vuex";
  import {
